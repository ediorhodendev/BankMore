...
public sealed class CreateMovementCommandHandlerTests
{
    ...
    public async Task Handle_Should_Create_Movement_For_Logged_Account()
    {
        ...
        var result = await handler.Handle(
            new CreateMovementCommand("req-1", null, 100, "C"),
            CancellationToken.None);

        result.IsSuccess.Should().BeTrue();

        movementRepository.Verify(
            x => x.AddAsync(It.IsAny<BankMore.Account.Domain.Entities.Movement>(), It.IsAny<CancellationToken>()),
            Times.Once);
    ...
    public async Task Handle_Should_Fail_When_Value_Is_Invalid()
    {
        ...
        var result = await handler.Handle(
            new CreateMovementCommand("req-1", null, 0, "C"),
            CancellationToken.None);
    ...
    [Fact]
    public async Task Handle_Should_Fail_When_Debit_Is_To_Another_Account()
    {
        ...
        var result = await handler.Handle(
            new CreateMovementCommand("req-1", "2222222222", 50, "D"),
            CancellationToken.None);
